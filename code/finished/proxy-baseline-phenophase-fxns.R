### Title: Helper Fxns for Proxy Baseline PhenoPhase
### Author: JT Miller
...
model_baseline_onset_offset <- function(target_taxa,
  ...
  if(nrow(phen_estimate) > 3){ # min req to make env
    ...
    if(length(model_results) == 0){
      ...
      if(length(top_models) == 1 || use_single_model) {
        ...
        predict_onset_scaled_glm <- function(object, newdata = NULL, type = "response", ...) {
          if(!is.null(newdata)) {
        ...
        predict_onset_ensemble_glm <- function(object, newdata = NULL, type = "response", ...) {
          if(!is.null(newdata)) {
            newdata_scaled <- newdata
            for(pred in names(object$scaling_params)) {
              if(pred %in% names(newdata)) {
  ...
  if(nrow(phen_estimate) > 3){ # min req to make env
    ...
    if(length(model_results) == 0){
      ...
      if(length(top_models) > 1) {
        ...
        # use single model if high variability detected
        if(high_variability_found) {
          cat("\n⚠ High coefficient variability detected across ensemble.\n")
          cat("Selecting single best model instead of ensembling.\n")
          use_single_model <- TRUE
      ...
      if(length(top_models) == 1 || use_single_model) {
        ...
        # Custom predict for single scaled model
        predict_offset_scaled_glm <- function(object, newdata = NULL, type = "response", ...) {
          if(!is.null(newdata)) {
            newdata_scaled <- newdata
            for(pred in names(object$scaling_params)) {
              if(pred %in% names(newdata)) {
        ...
        # Custom predict method for ensemble
        predict_offset_ensemble_glm <- function(object, newdata = NULL, type = "response", ...) {
          if(!is.null(newdata)) {
            newdata_scaled <- newdata
            for(pred in names(object$scaling_params)) {
              if(pred %in% names(newdata)) {
  ...
  if(nrow(phen_estimate) > 3){
    ...
    return(list(
      ...
      env_onset_prediction_fxn_single_scaled = predict_onset_scaled_glm, 
      env_onset_prediction_fxn_ensemble_scaled = predict_onset_ensemble_glm,
      env_offset_prediction_fxn_single_scaled = predict_offset_scaled_glm, 
      env_offset_prediction_fxn_ensemble_scaled = predict_offset_ensemble_glm,
      data = phen_data,
    ...
    return(list(
      ...
      env_prediction_fxn_single_scaled = NULL, 
      env_prediction_fxn_ensemble_scaled = NULL,
      data = phen_data,
      phen_est_data = phen_estimate, 
